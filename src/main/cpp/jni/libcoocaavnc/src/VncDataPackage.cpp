    ...
    : m_protocolbuffer((const char*)rawdata, datasize)
{
    ...
    if (m_content.m_paramType != CMD_ARGUMENT_TYPE_NONE)
    {
        ...
        {
            Json::Reader reader;
			VNCLOGD("VncDataPackage m_paramType == CMD_ARGUMENT_TYPE_KEYVALUE %s", m_content.m_bufParam.c_str());
...
const std::string & VncDataPackage::getStringValue(const std::string & key) 
{  
    static std::string nullstring("");
	if (!m_content.m_jsonroot.isMember(key))
		return nullstring;
	if (m_content.m_paramType == CMD_ARGUMENT_TYPE_KEYVALUE)
	{
		VNCLOGD("getKeyValue key == %s", key.c_str());
		nullstring = m_content.m_jsonroot[key].asString();
		VNCLOGD("getKeyValue value == %s", nullstring.c_str());
	}      

    return nullstring;
};

int VncDataPackage::getIntValue(const std::string & key, int & value)
{
	if (!m_content.m_jsonroot.isMember(key))
	{
		return -1;
	...
	{
		VNCLOGD("getKeyValue key == %s", key.c_str());
		value = m_content.m_jsonroot[key].asInt();
		VNCLOGD("getKeyValue value == %d", value);
	}
	return 0;
}
